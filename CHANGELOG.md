...
### Added
- **Phase 0 spec — Observability & Budgets** (`docs/phase-0-observability.md`): the CFO layer for the AI org — an `agent_runs` + `agent_budgets` data model (RLS-locked), a `costOf` pricing module, a `runAgent` instrumentation seam (budget pre-check → run → capture usage → cost → log → alert), budget enforcement (org-level spawn cap + per-run token cap), Web Push threshold alerts, and a gated cockpit dashboard (`/api/ops/summary` + `/ops`). Buildable, with a step-by-step checklist; nothing built yet.
