...
### Added
- **AI pitch (Claude)**: on the prospect card, "✨ Draft with AI" generates a grounded door pitch — opener, likely objections + responses, who to ask for, and the strongest angle — via a server route (`/api/ai/pitch`, key server-side, behind the password gate). Cached per prospect in `localStorage` (instant/offline); falls back to the templated pitch when unconfigured. Defaults to `claude-sonnet-5`. See docs/ai-pitch.md. The card also drops the abstract lead-score chip and the corridor hint to keep the pitch the focus.
