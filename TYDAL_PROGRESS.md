...
5. 🔶 Offline-first (spec: [docs/offline-first.md](docs/offline-first.md)) — **Phase 1 (reads) + Phase 2 (writes) BUILT & pushed**: IndexedDB store/outbox (`lib/offline.ts`, 8 fake-indexeddb tests) + DataProvider read-through/SWR + optimistic outbox + flush on reconnect/foreground. **Phase 3 (sync-status indicator by the count pill) DEFERRED until the browser is connected to verify.** End-to-end offline UX (reads render offline, edits queue+sync) is built to spec + builds green but NOT yet device-verified.
