...
5. 📝 SPEC'D (awaiting sign-off) — Offline-first: IndexedDB cache + optimistic writes + outbox/flush. Full design in **[docs/offline-first.md](docs/offline-first.md)**. Store + outbox logic is unit-testable via fake-indexeddb (no browser); offline UX verification needs the browser/device. Open questions in the doc need answers before building.
